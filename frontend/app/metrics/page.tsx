...
function StatCard({ label, value, sub, accent }: StatCardProps) {
  return (
    <div className="p-6 rounded-2xl border border-[#1e2d22] bg-[#0d1610]">
      <div className="text-xs text-[#6b8f72] mb-2">{label}</div>
      <div className={`text-3xl font-bold font-mono ${accent ? "text-seed-400" : "text-[#e8f5eb]"}`}>
        {value}
      </div>
      {sub && <div className="text-xs text-[#3d5c44] mt-1">{sub}</div>}
    </div>
  );
}
...
export default function MetricsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#e8f5eb]">Metrics</h1>
        <p className="text-sm text-[#6b8f72] mt-1">Overview of your RFP pipeline</p>
      </div>
      ...
      {/* Score distribution */}
      {completed.length > 0 && (
        <div className="p-6 rounded-2xl border border-[#1e2d22] bg-[#0d1610] mb-10">
          <h2 className="text-sm font-semibold text-[#e8f5eb] mb-6">Score distribution</h2>
          <div className="flex items-end gap-3 h-28">
            {BUCKETS.map((b, i) => (
              <div key={b.label} className="flex-1 flex flex-col items-center gap-2">
                <div className="text-xs text-[#6b8f72]">{bucketCounts[i]}</div>
                <div
                  className="w-full rounded-t-md bg-seed-900/60 border border-seed-800/40 transition-all"
                  style={{ height: `${Math.round((bucketCounts[i] / maxBucket) * 80)}px`, minHeight: bucketCounts[i] > 0 ? "4px" : "0" }}
                />
                <div className="text-xs text-[#3d5c44] whitespace-nowrap">{b.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Recent analyses */}
      <div className="p-6 rounded-2xl border border-[#1e2d22] bg-[#0d1610]">
        <h2 className="text-sm font-semibold text-[#e8f5eb] mb-4">Recent analyses</h2>
        {recent.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-sm text-[#6b8f72] mb-4">No analyses yet.</p>
            <Link
              href="/analyze"
              ...
              <div key={rfp.id} className="py-3 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <Link
                    ...
                    className="text-sm text-[#e8f5eb] hover:text-seed-400 transition-colors truncate block"
                  >
                    {rfp.filename}
                  </Link>
                  <div className="text-xs text-[#6b8f72] mt-0.5">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {rfp.status === "completed" && (
                    <>
                      ...
                      <span className="text-sm font-mono font-bold text-[#e8f5eb] w-8 text-right">
                        {rfp.score}
                      </span>
                    </>
                  )}
                  {rfp.status === "processing" && (
                    <span className="text-xs text-[#6b8f72]">processing…</span>
                  )}
                  {rfp.status === "partial_failure" && (

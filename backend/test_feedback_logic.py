...
Uses an in-memory SQLite DB — no LLM calls, no external services.
"""
import sys, json
...
# Create a fresh in-memory DB for this test run
...
import services.feedback as fb_mod
...
def make_rfp(score=72, decision="BID", industry="healthcare", fit_overall="high",
...
rfp = make_rfp(score=72, decision="BID", industry="healthcare")
fb = record_feedback(db, rfp.id, "won", "2026-03-15", "Strong past performance")
...
rfp2 = make_rfp(score=58, decision="NO BID", industry="technology", fit_overall="low")
fb2 = record_feedback(db, rfp2.id, "lost", "2026-03-20", None)
...
rfp3 = make_rfp(score=45, decision="NO BID", industry="construction", fit_overall="low")
fb3 = record_feedback(db, rfp3.id, "no_bid", None, "Decided not to bid")
assert fb3.outcome == "no_bid"
assert fb3.result_date == date.today().isoformat()  # defaults to today
print(f"Case 3 PASS: 'no_bid' with default today date = {fb3.result_date}")


# ── Case 4: invalid outcome → ValueError ─────────────────────────────────────
try:
    record_feedback(db, rfp.id, "maybe", "2026-03-01", None)
...
try:
    record_feedback(db, rfp.id, "won", "not-a-date", None)
    assert False
except ValueError as e:
...
records = get_feedback_for_rfp(db, rfp.id)
assert len(records) == 1
assert records[0]["outcome"] == "won"
assert records[0]["rfp_id"] == rfp.id
# rfp2 records should not appear
records2 = get_feedback_for_rfp(db, rfp2.id)
...
assert _win_rate(rows_mixed_nobid) == 1.0  # no_bid excluded from win-rate denominator
...
# score=72 (70-79 band) → won, score=58 (50-59 band) → lost
...
# Clear DB and test with 0 records
db2 = Session()
empty_summary = get_feedback_summary(db2)
...
# Win rate in 50-59 band >= 0.50 means many winners are below threshold
...
band_rates_low_threshold = {
    "60-69": {"win_rate": 0.20, "count": 5},  # many losses just above threshold
}
rows_losses_above = [make_feedback_row("lost", 65)] * 4 + [make_feedback_row("won", 65)] * 1
...
assert "raise" in result["message"].lower() or "higher" in result["message"].lower() or "raising" in result["message"].lower()
...
# Even if we can't find the rfp later, the snapshot fields are persisted
fb_check = db.query(Feedback).filter(Feedback.rfp_id == rfp.id).first()
...
# ── Case 15: record feedback for non-existent rfp (rfp_id not in DB) ─────────
fb_orphan = record_feedback(db, "rfp-that-does-not-exist", "lost", "2026-01-01", "Pre-existing record")
assert fb_orphan.rfp_id == "rfp-that-does-not-exist"
assert fb_orphan.original_score is None    # snapshot is empty but record exists
